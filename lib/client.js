 ...
 *
 * ⚠️ **只读（无例外）**：不点按钮、不改 openState、不替换原生 API、**不 monkey-patch 任何东西**。
 *    只用浏览器自带的只读观察器：MutationObserver（DOM）+ PerformanceObserver（网络）。
 *
 * ⚠️ 更正（2026-09-27，安全审查发现的"文档比实现更吓人"）：代码里**预留**了一个可选钩子
 *    `installReplaceHook()`（被调用时才会原地包装 `ClientAssistantStream.prototype.replace`），
 *    但**当前版本没有任何调用点** ⇒ 上面那句「无例外」成立；代价是第三类 `replace-failed` 记录
 *    **不会产生**。要启用须自行加调用 —— 那才会真的 monkey-patch，且**该路径未经安全审查**。
 *
 * 两件事：
	...
	factory: (require) => {
		...
		function readOpenState(sessionKey) {
			try {
				 ...
				 *
				 *  ⚠️ **诚实声明（H4 + 2026-09-27 更正）**：⚠️ **当前版本没有任何调用点 ⇒ 下面这次 monkey-patch 不会发生**，第三类 `replace-failed` 记录也不会产生。若被调用，这**是**一次 monkey-patch —— 它会**原地包装**
